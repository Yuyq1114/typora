...
- 使用 sync.WaitGroup 时要明确，所加的计数器到底是什么，比如对数据库数据加的时候，要使用

- 只要主程序（即 `main()` 函数对应的主 goroutine）没有退出或整个进程没有崩溃，那么你在任意函数中启动的 goroutine 都会继续执行，即使它们的“启动函数”早已返回。

- 协程在某个函数中调用时，函数结束协程不会关闭，因此在函数中的协程需要有退出机制，一般使用给每个 goroutine 加上 `for { select { case <-ctx.Done(): return ... } }`。

- ctx的Done 信号什么时候会发送：1调用 context.WithCancel(ctx) 返回的 cancel 函数时（你必须手动调用 cancel 才会触发 Done）。2如果使用 context.WithTimeout 或 context.WithDeadline，在超时或过期时自动触发。3父 context 被取消时，子 context 自动 Done。

- 不止在使用指针时注意空指针问题，在使用数组时也要注意越界

- 涉及多个pod的，一定要注意对于时区的处理，如使用time.Now()  的时候，或者redis的Set的时候，可以使用UTC转化。eg：先获取了time.Now()

- Go 的 `sync.Map`（以及普通 map）在比较 key 是否相等时：

  > 是根据「键的内存地址」来判断，而不是根据结构体字段内容。

  ~~因此如果使用字符串当key是可以的，但是字符串数组就是不可以的。~~主要还是使用结构体时，要用a:=A{}, 而不是 a:=&A{}

- 对于有索引的代码，一定要len，不然会出现索引越界问题

- 对于数据库查找中需要使用   IN  ?    的情况，在非pg数据库中，需要额外处理？中的数量，否则数据量太大会造成。错误/另外pg数据库可以使用  = ANY(?)   但是 ？要使用pq.Array(?) 转化为pq数组才行
...
```
func MapSetToSlice[T comparable](m map[T]struct{}) []T {
...
func ToInt64Slice[T ~int | ~int32 | ~uint | ~uint32 | ~uint64](src []T) []int64 {
...
10. 关于错误处理，在函数栈的最外层打印日志，内层封装栈返回即可，防止日志杂乱

10. 关于大型结构体滥用行为，将“传入数据结构”和“数据库入库结构”彻底分离，或者“Schema Versioning” 机制（适合多人协作）

    在 struct 中引入版本号字段，例如：

    ```
    type BehaviorRecordV1 struct {...}
    type BehaviorRecordV2 struct {...}
    ```
    ...
    当数据库 schema 有变动时，升级版本号。

10. 如果有一个操作，在一个函数中，先删除数据库记录，然后更新缓存，然后发送redis消息。但是在中间某一步失败了，然后返回error，后面的没有执行，前面的执行了；这种场景下，

    **1、考虑补偿机制**，即每一步错误后都处理上一步导致的 **2、最终一致性**，使用消息队列来解耦各个操作，将操作分解为独立的、异步的事件 **3、业务逻辑的调整**，读写缓存和数据库的先后。

10. 用“**卫语句+ 早期返回**”

    ```cpp
    if (!valid) return;        // 卫语句先挡掉异常流
    if (!exist)  return;
    ```



17.业务层里需要好多条件查一条数据，也应该将这个查询语句封装为 repo 函数并传入多个参数，即使只在当前使用。并且model层和数据库操作层也需要分开。`model` 只放 **数据结构**，不要放数据库操作。

文件按 **逻辑模块/领域** 分组，而不是按 struct 一一对应。

每个文件内可以有多个相关 struct。



18.对于数据库缓存不一致时，需要查询数据库来更新缓存的情况，有时候从数据库读到的结果是空，然后条件中或者循环就会跳过这个处理，要格外注意。实际解决1、新建中间缓存重新复写  2、给每个缓存设置一个flag，读到的就变动，没变动的缓存记录删除

| 方案                             | 适用场景              | 推荐  |
| -------------------------------- | --------------------- | ----- |
| 写空值缓存（Cache Null）         | 最常见、简单有效      | ⭐⭐⭐⭐⭐ |
| 统一的 GetOrLoad 函数            | 工程大型化时·防止遗漏 | ⭐⭐⭐⭐  |
| 双写补偿机制（写 → MQ → 刷缓存） | 高一致性需求场景      | ⭐⭐⭐⭐  |
| 默认值代替空值                   | 避免 nil 跳过逻辑     | ⭐⭐⭐   |

 旁路缓存策略：先更新数据库，再删除缓存。L1L2缓存
...
3、测试时，正向用例结束后，要更多的考虑反向用例。

4、`%w` 用于 **包装错误**（Go 1.13+）；

`fmt.Errorf("msg: %w", err)` 让错误链保留完整堆栈；

外层可以用 `errors.Is` 或 `errors.As` 判断错误类型；

只在最外层**最外面能返回error的层**打印一次，保持日志整洁。

Go 的错误传递机制鼓励 “错误即返回值”，通过逐层 `return err` 或 `fmt.Errorf("%w")`， 让最外层拥有足够上下文，决定如何打印或恢复，而不是在每一层都打印。
