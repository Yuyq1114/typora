...
1. 变量与方法统一使用驼峰命名风格
2. 提交代码前必须使用gofmt格式化代码，然后使用golint检查代码
3. json字符串建议使用反单引号(`)，任何可能其中出现转义字符的，都应使用反单引号
4. 文件名必须小写，允许下划线’_’，但头尾不能。
5. 目录名必须小写，允许中划线’-’，但头尾不能
6. 包名必须全部小写，无下划线，越短越好，禁止与标准库重名，禁止通过中划线连接多个单词
7. 函数名和结构体名必须为大小写驼峰模式，最好不带特殊字符如划线等
8. 常量和枚举名，大小写驼峰法，不允许下划线，第三方包例外
9. 函数参数首字母小写，不能有下划线，按大小驼峰法
10. 变量名不允许下划线，大小写驼峰法，局部变量首字母小写，全局变量首字母大写
11. 接口名大小写驼峰法，首字母大写，不能下划线，名词
12. 多个互相关联的全局变量在定义时使用括号集中，或者使用结构体



# 代码组织目录

1. 一个目录只包含一个包，复杂模块拆分子模块/目录
2. 非测试文件(*_test.go)禁止使用简化包
3. 禁止相对路径导入包
4. 模块中禁止导入不使用的包
