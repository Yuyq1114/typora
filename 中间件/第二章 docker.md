...
| 分类     | Docker对象                     | 解决什么问题         |
| -------- | ------------------------------ | -------------------- |
| 运行     | Container（容器）              | 程序怎么运行         |
| 镜像     | Image（镜像）                  | 程序运行需要什么环境 |
| 构建     | Dockerfile                     | 如何制作镜像         |
| 存储     | Volume（数据卷）               | 数据如何持久化       |
| 网络     | Network（网络）                | 容器之间如何通信     |
| 配置     | Environment Variables / Config | 配置如何注入         |
| 生命周期 | start / stop / restart         | 容器如何管理         |
| 管理     | Label（标签）                  | 如何分类和筛选       |
| 资源限制 | CPU / Memory Limit             | 如何限制资源         |
| 安全     | User / Capability              | 容器权限控制         |
| 发布     | Registry                       | 镜像如何存储和分发   |
...
| 名称       | 是什么          | 作用                                      |
| :--------- | :-------------- | :---------------------------------------- |
| K8s        | 容器编排平台    | 调度 Pod、管理网络存储、生命周期          |
| k3s        | 轻量 K8s 发行版 | 更容易安装的小 K8s                        |
| Docker     | 容器平台        | 本地跑容器、构建镜像，曾常被 K8s 间接使用 |
| containerd | 容器运行时      | 真正负责创建和管理容器                    |
| runc       | 底层执行器      | 调用内核创建容器                          |
| CRI        | 标准接口        | 让 K8s 能对接 containerd/CRI-O            |
