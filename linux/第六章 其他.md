...
1. **设置 `JAVA_HOME` 环境变量**
   ...
   - 获取路径后，设置 
     ...
     ```
     bash复制代码export JAVA_HOME=/path/to/java
   ...
   - 例如，如果 Java 安装在 
     ...
     ```
     bash复制代码export JAVA_HOME=/usr/lib/jvm/java-1.8.0-openjdk
     export PATH=$JAVA_HOME/bin:$PATH
     ```
   ...
   - 将这些行添加到 
     ...
     ```
     bash复制代码echo "export JAVA_HOME=/usr/lib/jvm/java-1.8.0-openjdk" >> ~/.bashrc
...
写入路由表，router add -net 10.45.0.0  netmask 255.255.0.0 gw 10.45.116.254
